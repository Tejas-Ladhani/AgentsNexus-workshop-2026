...
In MCP Inspector, confirm that the server exposes 13 tools, the `support_reply`, `ticket_update`, and `incident_summary` prompts, and the `customer://orders/{order_id}`, `customer://customers/{customer_id}`, and `customer://services/status` resources. The first two are resource templates. Explore `list_services`, `list_customer_ids`, and `list_orders` first, then call `lookup_order` with `ORD-1001`; the response should include `CUST-100`, `shipped`, and `89.00`.
